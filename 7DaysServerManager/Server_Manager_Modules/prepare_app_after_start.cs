    ...
    public partial class Server_Panel_Form : Form
    {
        ...
        private void Prepare_app_after_start()
        {
            ...
            */
            ...
            if (File.Exists("server_updater.exe"))
            {
                try
                {
                    ...
                    Echo("Update Was Successful!", 0, true);
                    MessageBox.Show(whatsnew, "What's New In 7 Days Server Manager " + ver);
                }
                catch
                {
                    MessageBox.Show("Can not remove server_updater.exe. Remove it manually.", "Error");
                }
            ...
            if ((string)Registry.GetValue(@"HKEY_CURRENT_USER\Software\7DSM", "install_id", null) == null)
            {
                long min = 10000000000001;
                long max = 99999999999999;
                Random random = new Random();
                long randomNumber = min + random.Next() % (max - min);
                Registry.SetValue(@"HKEY_CURRENT_USER\Software\7DSM", "install_id", randomNumber);
            }
            ...
            */
            ...
            // Getting Application Settings From the Registry And Updating UI Values
            ...
            try
            {
                ...
                auto_backup_group.Text = "Backup every" + " [" + hrs_s + ":" + min_s + " h.]";

            }
            catch { auto_backup_group.Text = "Backup every"; }
            ...
            try
            {
                ...
                Auto_Restarts_GroupBox.Text = " [" + Convert.ToString(Auto_Restart_Time_TrackBar.Value) + ":00]" + " [" + hrs_s + ":" + min_s + " h.]";
            }
            catch { Auto_Restarts_GroupBox.Text = " [" + Convert.ToString(Auto_Restart_Time_TrackBar.Value) + ":00]"; }
            ...
            // Init Android Access Key Variable
            string aak = "";

            // Generate Android Access Key (If It Doesn't Already Exist)
            try
            {
